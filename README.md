# Weather App

**Project**: A small client-side weather app that displays current weather information for a chosen location using Visual Crossing Weather API. The app is built with plain HTML, CSS and JavaScript and is intended as a lightweight demo project.

**Quick Start**
- **Open**: Double-click `index.html` in the project root to open in your browser.
- **Local server (recommended)**: Run a simple HTTP server and open `http://localhost:8000`.

Run a temporary server from PowerShell:
```powershell
python -m http.server 8000
```

Then open `http://localhost:8000` in your browser.

**Files**
- **`index.html`**: App entry page.
- **`styles/styles.css`**: Styles for the UI.
- **`script/index.js`**: Main JavaScript code and API interactions.
- **`images/`**: Static images used by the app.

**Development**
- Edit files in your editor of choice (e.g., VS Code).
- For quick reloads use the VS Code Live Server extension or the command above.
- Ensure you have an internet connection for any external API calls the app may perform.